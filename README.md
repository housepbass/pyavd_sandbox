...
4. Update the `./clab_topologies/l2ls.clab.yml` with your [preferred ceos image version](https://containerlab.dev/manual/kinds/ceos/)
5. Run `make deploy_clab` do deploy the eos containers via [Containerlab](https://containerlab.dev/)
6. Run `make deploy_avd` to execute the `deploy.yml` ansible playbook. **You might have to run this twice due to initial management VRF configuration changes.**
